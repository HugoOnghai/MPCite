...
def update_existing_osti_record(
    ...
    for keyword in new_values.keys():
        try:
            ...
            print(
                "Extraneous keywords found in the dictionary that do not correspond to attributes in the ELink API's record class."
            )
...
def delete_osti_record(
    ...
    response = requests.delete(
        f"{review_api.target}records/{osti_id}?reason={reason}",
        headers={"Authorization": f"Bearer {review_api.token}"},
    )
